...
export function getSyncPoints(state:TopStateType):Array<[number, number, number, number]> {
    return (state.selectedMap !== undefined) ? ( state.mapInfo[state.selectedMap].leftMap?.points || [] ) : []
}

function getPairPointListForLeft(points:Array<[number, number, number, number]>):Array<PointPair> {
    return points.map((point)=>[ [ point[0], point[1] ], [ point[2], point[3] ]])
}

function getPairPointListForRight(points:Array<[number, number, number, number]>):Array<PointPair> {
...
function updateArray<T>(ary:Array<T>, index:number, elem:T):Array<T> {
